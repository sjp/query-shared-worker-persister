...
import type { StorageResponse } from "./protocol";
...
async function loadWorker() {
  const scope: { onconnect: ((event: { ports: readonly WorkerPort[] }) => void) | null } = {
    onconnect: null,
  };
  vi.resetModules();
  ...
  if (!onconnect) {
    throw new Error("the worker installed no onconnect handler");
  }

  /** Fire a connect event, as the browser does, carrying the ports it names. */
  const fire = (ports: WorkerPort[]) => {
    onconnect({ ports });
  };
  ...
  const connect = () => {
    ...
    return {
      responses,
      /**
       * Post a request the way a connected tab would, and read the reply. Typed
       * as `unknown` because a real sender is not obliged to speak the protocol.
       */
      send(request: unknown) {
        port.onmessage?.(messageEvent(request));
